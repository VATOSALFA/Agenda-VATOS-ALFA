/**
 * Importamos las funciones de la Versi贸n 2 (Gen 2)
 */
...
const { onSchedule } = require("firebase-functions/v2/scheduler");        // Para cron jobs (recordatorios, cumplea帽os)
...
async function sendTwilioTemplate(to, contentSid, variables) {
    ...
    // Aseguramos formato MX (+521 para m贸viles)
...
/**
 * =================================================================
 * AUTOMATIZACI脫N 1: NOTIFICACI脫N DE NUEVA CITA (Inmediata)
 * =================================================================
 */
...
/**
 * =================================================================
 * AUTOMATIZACI脫N 2: RECORDATORIO DE CITA (Programado - Cada Hora)
 * =================================================================
 */
...
/**
 * =================================================================
 * AUTOMATIZACI脫N 3: CUMPLEA脩OS (Programado - Diario 9:00 AM)
 * =================================================================
 */
...
/**
 * =================================================================
 * AUTOMATIZACI脫N 4: OPINI脫N GOOGLE (Programado - Diario 10:00 AM)
 * =================================================================
 */
exports.sendReviewRequests = onSchedule({
    ...
    console.log(`[Cron] Solicitudes de rese帽a enviadas: ${sentCount}`);
});
...
exports.createPointPayment = onCall({ cors: true, secrets: [mpAccessToken], invoker: 'public' }, async (request) => {
    ...
    try {
        ...
        if (!projectId) throw new HttpsError('failed-precondition', 'Could not determine Project ID for Webhook URL.');

        const notificationUrl = `https://us-central1-${projectId}.cloudfunctions.net/mercadoPagoWebhook`;
        // Console log for debugging
        console.log(`[Dynamic URL] Setting notification_url to: ${notificationUrl}`);
        ...
        if (!response.ok) {
            if (response.status === 409) throw new HttpsError('aborted', 'La terminal est谩 ocupada. Cancela la operaci贸n en el dispositivo.');
...
// 4. MERCADO PAGO WEBHOOK (V14 - SOLUCI脫N TOTAL)
...
/**
 * =================================================================
 * AUTOMATIZACI覰 5: SINCRONIZACI覰 DE ELIMINACI覰 DE USUARIO
 * =================================================================
 */
exports.onUserDeleted = onDocumentDeleted("usuarios/{userId}", async (event) => {
    ...
    try {
        ...
        if (error.code === 'auth/user-not-found') {
            console.log(\[Auth] Usuario \ ya no exist韆 en Auth.\);
        } else {
