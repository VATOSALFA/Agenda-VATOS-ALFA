...
async function saveMessage(from: string, body: string | null, mediaUrl: string | null, mediaType: string | null) {
    ...
    // Check if conversation exists to avoid overwriting clientName
    ...
    if (convSnap.exists()) {
        ...
        await conversationRef.set({
            ...
            lastMessageTimestamp: FieldValue.serverTimestamp(),
            unreadCount: FieldValue.increment(1),
            // clientName can be added here if available from another source
        });
    }
}
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!from) {
            console.error('Twilio webhook missing "From" parameter.');
