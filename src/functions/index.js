...
        
        // Intenta obtener dataId de query (para simulación/legacy) o body (para notificaciones reales)
        const dataIdQuery = request.query['data.id']; // ID en query param (usado para la firma/simulación)
        const dataIdBody = request.body.data?.id; // ID en el cuerpo JSON
        
        // Usamos el ID del cuerpo para el recurso, pero el de la query para la firma si está presente
        const dataIdForSignature = dataIdQuery || dataIdBody; 
        const resourceId = dataIdBody;

        console.log(`[Webhook] data.id (for signature) found: ${dataIdForSignature}`);
...
        
        // --- INICIO CORRECCIÓN DE MANIFEST ---
        // Construir el manifest estrictamente según la documentación (removiendo si no están).
        let manifestParts = [];
        
        // Usamos dataIdQuery para el ID si existe, forzando minúsculas.
        // Si solo viene en el body (dataIdBody) no se usa en el template 'id' según la documentación de MP.
        // Si estás usando el simulador, asegúrate de poner un ID en el campo de identificación.
        if (dataIdQuery) {
            manifestParts.push(`id:${String(dataIdQuery).toLowerCase()}`);
        } else if (dataIdBody) {
            // Fallback para asegurar que el ID de la orden se incluya si no está en la query (algunos tópicos).
            manifestParts.push(`id:${String(dataIdBody).toLowerCase()}`);
        }
...
        // --- INICIO CORRECCIÓN LÓGICA DE ORDER.PROCESSED ---
        if (body.type === 'order' && body.action === 'order.processed') {
             console.log("[Webhook] Received 'order' type webhook. Fetching Order details...");
             const orderId = resourceId; // Usamos el ID del cuerpo para consultar la API
            
             if (!orderId) {
                 console.log("[Webhook] Webhook received for processed order without data.id in body.");
             } else {
                 const { accessToken } = await getMercadoPagoConfig();
...
                 // Una orden de Point se considera pagada cuando su status es 'closed'
                 if (externalReference && orderData.status === 'closed') { 
...
                     }
                 } else {
                     console.log("[Webhook] Order not closed or no external reference found.", orderData);
                 }
             }
        } 
        // --- FIN CORRECCIÓN LÓGICA DE ORDER.PROCESSED ---
...
        // Respondemos con 200 OK para evitar que MP reintente continuamente.
        response.status(200).send("OK_WITH_ERROR"); 
        return;
    }
    
    console.log("===================================================");
    // RESPUESTA ESPERADA POR MERCADO PAGO
