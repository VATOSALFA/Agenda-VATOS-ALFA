...
export default function ConversationsPage() {
    ...
    // Handle URL query param for phone
    useEffect(() => {
        if (phoneParam && !conversationsLoading) {
            const cleanPhoneParam = phoneParam.replace(/\D/g, '');
            const phone10 = cleanPhoneParam.slice(-10);

            const found = conversations.find(c => {
                const cPhone = c.id.replace(/\D/g, '');
                return cPhone.slice(-10) === phone10;
            });
    ...
    }, [phoneParam, conversations, conversationsLoading]);
