...
export function EmailPreview({ config, type, companyDetails }: EmailPreviewProps) {
    ...
    const getHtml = () => {
        ...
        const senderName = companyDetails?.name || "VATOS ALFA Barber Shop";
        const logoUrl = companyDetails?.logo || "https://firebasestorage.googleapis.com/v0/b/agenda-1ae08.appspot.com/o/empresa%2Flogo.png?alt=media";
        const secondaryColor = companyDetails?.secondaryColor || "#314177";
        const dateStr = "Lunes, 12 de Octubre, 2026";
        const timeStr = "10:00 AM";
        const professionalName = "Carlos Barbero";
        const localAddress = companyDetails?.address || "Av. Cerro Sombrerete 1001";
        const localPhone = companyDetails?.phone || "442-123-4567";
        const whatsappLink = "#";
        ...
        if (type === 'confirmation') {
            ...
            const footerNote = (config.confirmationEmailNote || 'Favor de llegar 5 minutos antes de la hora de tu cita.').replace(/\n/g, '<br/>');
            ...
            <link href="https://fonts.googleapis.com/css2?family=Roboto:wght@400;700&display=swap" rel="stylesheet">
                ...
                <div style="background-color: #ffffff; padding: 15px 25px 10px 25px; text-align: center;">
                    <img src="${logoUrl}" alt="${senderName}" style="display: block; margin: 0 auto; width: 100%; max-width: 260px; height: auto; object-fit: contain;" />
                </div>

                <div style="padding: 20px 25px 25px 25px;">
                    <h2 style="color: ${secondaryColor}; text-align: center; margin-top: 0px; margin-bottom: 5px; font-family: 'Roboto', Arial, sans-serif; font-weight: 700; font-size: 24px; line-height: 1.2;">${headline}</h2>
                    
                    <div style="margin-bottom: 20px; text-align: center;"><div style="margin-bottom: 4px; font-family: 'Roboto', Arial, sans-serif; font-size: 1.4em; font-weight: 700; color: #333;">Corte de Cabello</div></div>
                    ...
                    <div style="background-color: #ffffff; color: #333; padding: 15px; border-radius: 8px; font-size: 0.9em; margin-top: 15px; text-align: center; border: 1px solid #000000;">
                        ${footerNote}
                    </div>
                    ...
                    <div style="margin-top: 20px; text-align: center;">
                        <div style="display: inline-block; width: 100%;">
                            ${config.showConfirmAction ? `
                            <a href="#" style="display: inline-block; background-color: #22c55e; color: white; padding: 8px 12px; border-radius: 6px; text-decoration: none; font-size: 11px; font-weight: bold; margin: 4px;">CONFIRMAR</a>` : ''}
                            ${config.showRescheduleAction ? `
                            <a href="#" style="display: inline-block; background-color: #314177; color: white; padding: 8px 12px; border-radius: 6px; text-decoration: none; font-size: 11px; font-weight: bold; margin: 4px;">REAGENDAR</a>` : ''}
                            ${config.showCancelAction ? `
                            <a href="#" style="display: inline-block; background-color: #ef4444; color: white; padding: 8px 12px; border-radius: 6px; text-decoration: none; font-size: 11px; font-weight: bold; margin: 4px;">CANCELAR</a>` : ''}
                        </div>
                    </div>` : ''}
                ...
                <div style="background-color: #ffffff; padding: 20px; text-align: center; font-size: 0.75em; color: #bbb; border-top: 1px solid #f9f9f9;">
                    ${(config.signature || senderName).replace(/\n/g, '<br/>')}
                </div>
            </div>
        </div>`;
            ...
            const footerNote = (config.reminderFooterNote || 'Te esperamos 5 minutos antes de tu cita.').replace(/\n/g, '<br/>');
            ...
            return `
               <div style="font-family: 'Roboto', Arial, sans-serif; color: #333; max-width: 100%; padding: 20px;">
                ...
                <div style="max-width: 400px; margin: 0 auto; background-color: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
                   ...
                   <div style="padding: 20px 25px 25px 25px;">
                        <h2 style="color: #333; text-align: center; margin-top: 0px; margin-bottom: 2px; font-family: 'Roboto', Arial, sans-serif; font-weight: 700; font-size: 24px; line-height: 1.2;">${headline}</h2>
                        <p style="text-align: center; color: #999; font-size: 0.9em; margin-bottom: 20px;">${subHeadline}</p>
                        
                        <div style="margin-bottom: 20px; text-align: center;">
                            <div style="margin-bottom: 4px; font-family: 'Roboto', Arial, sans-serif; font-size: 1.4em; font-weight: 700; color: #333;">Corte de Cabello</div>
                        </div>
                        ...
                        <div style="background-color: #ffffff; color: #333; padding: 15px; border-radius: 8px; font-size: 0.9em; margin-top: 20px; text-align: center; border: 1px solid #000000;">
                           ${footerNote}
                        </div>

                        ${config.enableReminderQuickActions ? `
                        <div style="margin-top: 20px; text-align: center;">
                            <div style="display: inline-block; width: 100%;">
                                ${config.showReminderConfirmAction ? `
                                <a href="#" style="display: inline-block; background-color: #22c55e; color: white; padding: 8px 12px; border-radius: 6px; text-decoration: none; font-size: 11px; font-weight: bold; margin: 4px;">CONFIRMAR</a>` : ''}
                                ${config.showReminderRescheduleAction ? `
                                <a href="#" style="display: inline-block; background-color: #314177; color: white; padding: 8px 12px; border-radius: 6px; text-decoration: none; font-size: 11px; font-weight: bold; margin: 4px;">REAGENDAR</a>` : ''}
                                ${config.showReminderCancelAction ? `
                                <a href="#" style="display: inline-block; background-color: #ef4444; color: white; padding: 8px 12px; border-radius: 6px; text-decoration: none; font-size: 11px; font-weight: bold; margin: 4px;">CANCELAR</a>` : ''}
                            </div>
                        </div>` : ''}
            ...
            const footerNote = (config.profConfirmationEmailNote || '').replace(/\n/g, '<br/>');
