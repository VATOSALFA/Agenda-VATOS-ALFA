...
async function getMercadoPagoConfig() {
    ...
    if (!accessToken || !userId || !webhookSecret) {
        throw new functions.https.HttpsError('failed-precondition', 'La configuración de Mercado Pago (AccessToken, UserID o WebhookSecret) no está completa.');
    }
    return { accessToken, userId, webhookSecret };
}

const MP_API_BASE = 'https://api.mercadopago.com';

function validateMercadoPagoSignature(req: functions.https.Request, secret: string): boolean {
    const signatureHeader = req.headers['x-signature'] as string;
    if (!signatureHeader) {
        functions.logger.warn('MP Webhook: Missing x-signature header.');
        return false;
    }
...
export const mercadoPagoWebhook = functions.runWith({ secrets: ["MERCADO_PAGO_WEBHOOK_SECRET"] }).https.onRequest(
    async (request, response) => {
        const { webhookSecret, accessToken } = await getMercadoPagoConfig();
        if (!validateMercadoPagoSignature(request, webhookSecret)) {
